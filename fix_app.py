...
# 백업 생성 (기존 파일이 있다면)
existing_files = [f for f in os.listdir('.') if f.endswith('.py') and 'fix_app' not in f and 'composer_utils' not in f]
if existing_files:
...
def load_settings():
    try:
        ...
        return {
            ...
            "APP_VERSION": settings.get("app_version", "v1.2"),
...
with left_col:
    ...
    st.button(
        ...
        use_container_width=True, 
        key="clear_templates",
        on_click=clear_template_images,
...
try:
    with open("app.py", "w", encoding="utf-8") as f:
        f.write(fixed_code)
    print("🎉 성공! app.py 파일이 새로 생성되었습니다.")
    print("✅ 삭제 버튼 상호 비활성화 문제가 해결되었습니다.")
    print("✅ 다운로드 후 미리보기 새로고침 문제가 해결되었습니다.")
    print("\n이제 'streamlit run app.py'를 실행하시면 됩니다.")
